/***************************************
 * MAIN (efeitos simples)
 ***************************************/
document.addEventListener("DOMContentLoaded", () => {
  const menuLinks = document.querySelectorAll(".menu a");
  const path = location.pathname.split("/").pop() || "index.html";
  menuLinks.forEach(l => { if (l.getAttribute("href") === path) l.classList.add("active"); });
});
/***************************************
 * I18N (traduções globais)
 ***************************************/
const I18N={
  en:{nav:{home:"Home",about:"About",projects:"Projects",contact:"Contact"},
      ...
      proj:{music:{title:"Laia Music",desc:"Preserving and spreading Edney Fernandes' musical legacy.",btn:"Open"},
  ...
  pt:{nav:{home:"Início",about:"Sobre",projects:"Projetos",contact:"Contato"},
      ...
      proj:{music:{title:"Laia Music",desc:"Preservando e difundindo o legado musical de Edney Fernandes.",btn:"Abrir"},
      ...
      about:{p1:"Oi! Sou Gabriela Barbosa de Sousa, do Brasil. Falo português, espanhol e pratico inglês.",
  ...
  es:{nav:{home:"Inicio",about:"Sobre mí",projects:"Proyectos",contact:"Contacto"},
      ...
      proj:{music:{title:"Laia Music",desc:"Preservando y difundiendo el legado musical de Edney Fernandes.",btn:"Abrir"},
...
function getLang(){const saved=localStorage.getItem("lang");if(saved&&I18N[saved])return saved;const nav=(navigator.language||"en").slice(0,2);return I18N[nav]?nav:"en"}
function setLang(code){if(!I18N[code])return;localStorage.setItem("lang",code);applyTranslations(code)}
function text(el,t){if(el)el.textContent=t}
function applyTranslations(lang=getLang()){
  const d=I18N[lang];
  ["home","about","projects","contact"].forEach(id=>{
    const el=document.querySelector(`#nav-${id}`); if(el) el.textContent=d.nav[id];
  });
  $$("[data-i18n]").forEach(el=>{
    const key=el.getAttribute("data-i18n");
    const parts=key.split("."); let cur=d;
    for(const p of parts){ if(cur && p in cur) cur=cur[p]; else {cur=null; break;} }
    if(typeof cur==="string") el.textContent=cur;
  });
  const h1=document.getElementById("hero-1"), p2=document.getElementById("hero-2");
  if(h1) h1.textContent=d.hero1; if(p2) p2.textContent=d.hero2;
}
function initLangButtons(){ $$("[data-lang]").forEach(btn=>btn.addEventListener("click",()=>setLang(btn.dataset.lang))); }
document.addEventListener("DOMContentLoaded", ()=>{ const initial=getLang(); localStorage.setItem("lang",initial); applyTranslations(initial); initLangButtons(); });
